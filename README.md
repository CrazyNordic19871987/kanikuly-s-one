...
Геймифицированная платформа для управления летними каникулами детей 7–12 лет. 11 тематических смен, 7 профессиональных направлений, Anglophone-среда, спорт, коллекционная система из **90 карточек**, DISC-профилирование и защита проектов перед родителями.
...
![Cards](https://img.shields.io/badge/cards-90-8B5CF6)
...
**Каникулы с ONE!** — это:
- 🏕️ **11 тематических смен** — от «Scout Tech» до «Кибер-Атлеты»
- 🗺️ **7 профессиональных направлений** — Спорт, IT, Биотех, Дипломатия, Медиа, Art & Design, Предпринимательство
- 🃏 **Коллекция из 90 карточек** — инвентарь (55), реликвии (11), баджи (5), боссы (4), магазин (8), таинственные сундуки (7)
...
## 🃏 Коллекция карточек (90 шт.)

Одна строка = одна игровая карточка. Разделы:

| Раздел | Кол-во | Описание |
|--------|:------:|----------|
| **inventory** | 55 | Предметы-усиления по миссиям (1–11) |
| **relic** | 11 | Легендарные реликвии «сквозной» смен (1–11) |
...
│   └── views-cards.js      ← Коллекция из 90 карточек
...
| `009–011` | Целостность, ключи редкостей, проверки/чистка |
| `012` | Удаление «дрон-миссии» |
| `013–015` | Auth-профили, привязка игроков, фикс RLS-рекурсии |
| `016–018` | `player_progress`, баннеры смен, загрузка аватаров |
| `019_security_hardening.sql` | Защита: эскалация роли + замок PII (2026-09-08) |
| `020_scout_tech_shift1.sql` | Scout Tech в Shift 1 (2026-09-15) |
| `021_cyber_athletes_shift11.sql` | Кибер-Атлеты → Shift 11 + карточки 85–90 (2026-09-15) |
...
cards              -- коллекция: 90 карточек (num, section, rarity, bonus, ...)
...
Все таблицы используют `Row Level Security`. Публичный банк детей (ФИО, возраст, заметки) закрыт: зарегистрированный игрок видит только своих детей (через `user_id`/username-claim), администратор — всё. Публичная регистрация отключена — учётные записи создаёт администратор.
...
|------------|-----|-------------|
| `--orange` (акцент) | `#3B82F6` | Кнопки, акценты, активные элементы |
| `--green` | `#FBBF24` | Легендарные/статичные статусы |
| `--purple` | `#8B5CF6` | Эпические элементы |
| `--sky` | `#93C5FD` | Вторичный синий |
| `--white` | `#F0EDE5` | Основной текст |

Замечание: имена `--orange`/`--green` не совпадают с фактическим цветом (синий/жёлтый) — планируется переименование в `--accent`/`--gold` при рефакторинге CSS.
